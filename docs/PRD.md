...
- **Visual target: the Grok CLI.** M4's look-and-feel target is xAI's Grok CLI (user requirement, 2026-09-09; primary evidence: `~/.grok/docs/user-guide/06-theming.md` + `03-keyboard-shortcuts.md`, verified from the local install). Concretely:
  ...
  - **Reference palette (GrokNight, the default):** neutral dark base `#0e0e0e`/`#1a1a1a`, magenta accent `#7D4BC6` (user/cursor via OSC 12), cyan `#0db9d7` (tool/running), neutral grays `#c8c8c8`/`#909090`/`#444444`; GrokDay = light counterpart. Ships the five built-ins (GrokNight, GrokDay, TokyoNight, RosePineMoon, OscuraMidnight) + `auto` (OS dark/light polling every 5 s, `GROK_THEME`-style env override) + `/theme` live-reload picker with real-time preview.
...
*Last updated: 2026-09-09 (MVP built: M0–M3 complete in print mode — go build/test/vet green, 11.3 MB RSS boot, live-streamed coding task via onegw; omp session interop test; TUI visual target = Grok CLI per §3.5; Grok spec + statuses on issues #1–#5)*
