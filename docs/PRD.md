...
| M7 | Ext subprocess protocol + FS-scan cache + AST shell-out | parity with omp daily workflow | **Partial (2026-09-11)** — FS-scan cache (canonical-keyed, TTL 1s/16 entries/200ms empty revalidation, invalidate-on-write/rename, producers wired), grep/glob (rg/fd fast path + cached pure-Go fallback, byte-identical output either way), ast_grep/ast_edit (shell out, staged by default), and the extension protocol (JSONL handshake, fail-closed per-call `tool_call` policy with dead-extension retirement, `tool_result` patch, per-event timeout + SIGKILL, live-verified). Consumers landed (c8f600d, 4234034): `@`-file completion over the shared cache, extension slash commands, declarative card/table/tree renderers with five test-pinned degrade paths, distinct followUp routing. Remaining: `aside` (no loop surface yet) and `register_provider` (needs the M9 registry) | [#8](https://github.com/FreePeak/xdev/issues/8) |
| M8 | Memory hardening audit, fuzzing, cross-platform builds, packaging | <100 MB RSS verified under worst-case transcript | **Audited + fixed (2026-09-11, ce8fcae)** — measured: 109 MB / 50k-entry session loaded to 130 MB heap (over budget). Fix: **windowed materialization** — entries before the latest reset/compaction boundary are dropped at load (indexes rebuilt in batches; file untouched) → 62 MB for a 104 MB transcript, pinned by `TestWorstCaseTranscriptStaysUnderBudget`. Fuzzers for the session loader and the JSON-repair parser (87k / 5.8M execs clean) + 20s fuzz in CI; `govulncheck` pinned (0 called vulns); release-artifacts job (6 static platforms + SHA256SUMS). Residual: a never-compacted session stays unbounded by design (M5 auto-compaction makes the shape unreachable) | [#9](https://github.com/FreePeak/xdev/issues/9) |
...
| M10 | Session UX: commands, lifecycle, context files, prompts, keybindings — slash commands (native markdown discovery project+user, quote-aware `$1..$n`/`$@`/`$ARGUMENTS` expansion, unknown fall-through); lifecycle `/new` `/fresh` `/clear` `/drop` `/fork` (fork = new file + parentSession header); `--continue` breadcrumb (TTY/pane-keyed) + `--resume` + picker + `switchSession` runtime (hooks, rollback, model/tier restore); `/tree` `/branch` selectors + labels/filters; context files (AGENTS.md/CLAUDE.md hierarchy, depth dedup, `@path` imports); SYSTEM.md/APPEND_SYSTEM.md/TITLE_SYSTEM.md/PERSONALITY.md + `--system-prompt`/`--append-system-prompt`; keybindings remap + `/hotkeys`; `/dump`; `/rename` + title cascade (custom > ai > first prompt); `/todo` command surface + markdown round-trip; context-file injection scan; 3-tier prompt-cache assembly | Continue/resume/fork/branch/clear workflow works; custom markdown commands expand; AGENTS.md + imports reach context | **Partial (2026-09-11)** — slices landed: slash commands (built-in registry + native markdown discovery + quote-aware arg expansion + unknown fall-through), lifecycle `/new` `/clear` `/drop`, and the resume/fork/dump slice 6a7d8e2: `-resume <id-prefix>` (case-insensitive prefix, mtime desc, errors on no match), breadcrumb-first `-continue` (pane-env/TTY keyed), TUI `/fork` (new file + `parentSession` header, entries verbatim, memory-only sessions materialized first), `/dump` (markdown transcript export), `/resume <prefix>` with live store swap + transcript replay; DONE (2026-09-11): `/tree` `/branch` (store.Tree/Branch, live transcript rewrite), `/resume <prefix>` live store swap + listing; `/model` lists + switches the active model (ModelChangeEntry, next-turn apply). remaining: `/fresh`, `/resume` interactive picker, `switchSession` runtime hooks, `/prewalk` — rest (context files + `@path` imports, SYSTEM.md variants, keybindings) landed in earlier slices | [M10](https://github.com/FreePeak/xdev/issues/11) |
| M11 | Agent system: task agents, hub, hooks, advisor, prewalk, plan mode — task-agent discovery (markdown+frontmatter, first-wins merge project→user→extension→bundled, invalid skipped w/ warning); spawn policy (allowlist, `task.maxRecursionDepth=2` depth guard, plan-mode read-only children, blocked self-recursion); model precedence (task > frontmatter > parent); hub tool (messaging/jobs/processes) + Agent Hub TUI roster; hooks event bus (session/agent/turn/tool lifecycle; fail-closed interception; command-style subprocess hooks); advisor/watchdog (per-delta steering, nit→aside/concern→interrupt/blocker→steer, NFKC guard, cooldown); prewalk (big→`@smol` handoff gated on todo, `--prewalk`/`/prewalk`); magic keywords (ultrathink/orchestrate/workflowz); plan mode + `xd://propose`/`xd://resolve`; parent-owned todo policy; cross-session mailbox messaging (NICE) | Parent spawns scoped subagent steering back via hub; advisor steers on concern; prewalk switches model after first edit | **Nearly complete (2026-09-11)** — landed: task-agent discovery (markdown+frontmatter, first-wins) + spawn policy + depth-2 recursion; magic keywords; prewalk (failover-composed handoff); hub (background jobs + send/wait/cancel/result); hooks event bus (shell-command hooks, fail-closed, interceptor chain); plan mode + propose/resolve; advisor runtime (delta feed, advise severities, emission guard, 3-failure halt) + `/advisor`. `/model` and `/plan` landed on the M10 surface. remaining (NICE): WATCHDOG.md/yml roster, hub TUI roster, embedded-JS hooks | [M11](https://github.com/FreePeak/xdev/issues/12) |
| M12 | Knowledge & chrome: memory, skills, theme, TUI — memory backend seam + local backend (two-phase: extraction → smol consolidation → MEMORY.md + learned.md; lease+heartbeat; bounded caps) + `memory://` read seam + `/memory`; skills (SKILL.md discovery native+custom+managed first-wins, `skill://` protocol, `/skill:` commands); `learn` tool (+ managed-skill create/update w/ authored-shadowing conflict); theme engine (**slot vocabulary = §3.5's Grok slot names — `bg_*`, `accent_user/assistant/thinking/tool/...`, `text_*`, `gray_*`, diff/markdown slots — the single vocabulary M4 and M12 share; omp's 66-token list maps onto these slots**; vars resolution, hex/256/terminal colors, truecolor/256 detection with quantization, auto dark/light via OS appearance + OSC 11, symbol presets, box-drawing overrides, live reload w/ last-good fallback; GrokNight + GrokDay shipped in M4, remaining built-ins + `auto` land here); TUI chrome (themed status-line/HUD + spinner, overlay/dialog components incl. ask picker, custom tool renderers) | Memory summary+lessons inject at session start; `/skill:` expands; theme live-reloads with all slots enforced | **Landed (2026-09-11)** — skills: SKILL.md discovery (project > user > managed, first-wins), `skill://` seam with traversal rejection, prompt listing with hidden/model-invocation-disabled filtering. Memory: local backend (MEMORY.md + learned.md), Memory Guidance injection, `learn` tool (lesson stored before any skill write), `memory://` seam, `/memory view|stats|clear`. Theme: custom JSON themes with grouped missing-slot validation, vars + camelCase compat, symbol presets, polling live reload, `/theme`. Residual NICE: mnemopi/hindsight backends, autolearn two-phase pipeline, marketplace/foreign skill providers | [M12](https://github.com/FreePeak/xdev/issues/13) |
