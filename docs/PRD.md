...
**Known gaps (user-reported 2026-09-10):**
1. **`/<command>` slash commands are not functional in the current build** — typing `/new`, `/clear`, etc. in the TUI does nothing; the editor treats `/…` input as a plain prompt. This is M10 scope ([#11](https://github.com/FreePeak/xdev/issues/11), not started). Expected behavior per §2 "Session UX": native markdown command discovery (project + user), quote-aware `$1..$n`/`$@`/`$ARGUMENTS` expansion, unknown-command fall-through, plus lifecycle commands `/new` `/fresh` `/clear` `/drop` `/fork`.
2. **TUI chat-session scrolling is missing** — the user cannot scroll back through the conversation. This is a **P0 usability blocker** (new issue, cuts into M4's frame-plan TUI scope); the terminal-owns-scrollback model requires explicit keyboard scrollback interaction to be daily-drivable.
3. **Agent stops mid-task with `error: agent: exceeded 32 turns`** — the hard `MaxTurns = 32` constant (`internal/agent/loop.go:85`) kills any legitimately long coding task. P0 [#18](https://github.com/FreePeak/xdev/issues/18): make the cap configurable (`agent.maxTurns`), end runs gracefully with a wrap-up prompt instead of a raw error, keep runaway-loop protection.
