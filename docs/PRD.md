...
- **Visual target: the Grok CLI.** M4's look-and-feel target is xAI's Grok CLI (user requirement, 2026-09-09; primary evidence: `~/.grok/docs/user-guide/06-theming.md` + `03-keyboard-shortcuts.md`, verified from the local install). Concretely:
  - **Theme slots** (superset of omp's 66-token system; all RGB-defined, quantized at startup to truecolor/256/16, `NO_COLOR` → monochrome): backgrounds `bg_base` `bg_light` `bg_dark` `bg_highlight` `bg_hover` `bg_terminal` `bg_visual`; accents `accent_user` `accent_assistant` `accent_thinking` `accent_tool` `accent_system` `accent_error` `accent_success` `accent_running` `accent_skill` `accent_plan` `accent_verify` `accent_remember` `accent_model`; text `text_primary` `text_secondary`; grays `gray_dim` `gray` `gray_bright`; semantic `command` `path` `running` `warning` `fuzzy_accent`; borders/scrollbar `selection_border` `hover_border` `prompt_border` `prompt_border_active` `scrollbar_bg` `scrollbar_fg`; paste `paste_bg` `paste_fg` `paste_dim`; diff `diff_delete_bg` `diff_delete_fg` `diff_insert_bg` `diff_insert_fg` `diff_equal_fg` `diff_gutter_fg`; markdown `md_heading_h1..h6` `md_code` `md_code_bg` `md_text` `md_muted` `md_task_checked` `md_task_unchecked` `link_fg`.
  - **Reference palette (GrokNight, the default) — indicative, not verified:** neutral dark base, one magenta accent (user/cursor via OSC 12), one cyan tool accent, neutral grays — per the theming doc's qualitative description; the specific hexes `#0e0e0e`/`#7D4BC6`/`#0db9d7`/`#c8c8c8` came from binary string extraction with no slot attribution and MUST be verified by rendering a real Grok session (pixel sampling) before M4 pins them. xdev defines its own exact hexes against the qualitative constraints (256-color-quantization-safe, per the doc). Built-ins: GrokNight + GrokDay at M4 (TokyoNight/RosePineMoon/OscuraMidnight + `auto` OS-appearance polling trail in M12 — a Grok behavior clone only if it stays cheap; the theme engine's own scope in M12 governs).
...
| M4 | TUI: frame-plan renderer, editor, history retirement, resize — **visual target: the Grok CLI** (GrokNight theme slots + accent-rail scrollback, folding, blocking cards; §3.5; parity = layout/styling/interaction/palette only — animation/hover/bg-blend off by default) | daily-drivable interactive mode that a Grok CLI user recognizes as the same product class | **Complete (2026-09-09, MVP scope)** — `xdev tui` runs the Grok-styled TUI end-to-end (theme engine with GrokNight/GrokDay + slot vocabulary + quantization; streaming blocks with accent rail, tool status, dimmed thinking; editor with history; status line with token counters + spinner; Esc-cancel/Ctrl+C; OSC 12 cursor; live-verified in tmux against onegw incl. tool execution + cancel + persistence). Deferred to #5: markdown rendering, vim mode, blocking cards, history retirement batching, DSR resize recovery (simplified re-wrap ships instead) | [#5](https://github.com/FreePeak/xdev/issues/5) |
...
*Last updated: 2026-09-10 (M5 MVP landed: typed `ai.HTTPError` + err-class taxonomy, transient pre-content retry ladder, threshold+overflow compaction with store-anchored `firstKeptEntryId` via `buildContext.EntryIDs`, steering persistence; TUI history now rebuilt from the store per submit — fixes cross-turn conversation amnesia (tmux-verified codeword recall); 401 on onegw/free root-caused to client-key drift between the two onegw.toml files after the gateway restart, fixed via `${ONEGW_KEY}` env + key convergence, not xdev code; M4 polish (markdown rendering) landed in parallel; tails: promotion ladder, failover chains, partial-stream retain+continue)*
