...
| M6 | RPC mode (wire-compatible-ish) + subagents (yield contract) + MCP client + deferred tool catalog | embedders can drive it | **Partial (2026-09-11)** — RPC mode shipped (9db3a7d): `xdev rpc` JSONL-over-stdio with ready handshake (protocol 1, 1 MiB frames), seven id-correlated commands, event streaming with the materialized done message, bounded concurrent dispatch; hermetic pipe tests + external-driver smoke (prompt→events→exact reply, steer/abort/state round-trips). Subagents shipped (b393086): `SpawnChild` yield contract + `task` tool (restricted tools, outputSchema permissive/strict, artifact handoff), yield-only isolation test-pinned, child sessions isolated by `titleSource:subagent` + a dedicated subtree so resume never adopts one; live-verified delegation. Remaining: MCP client, host-tool/dialog frames (v2) | [#7](https://github.com/FreePeak/xdev/issues/7) |
