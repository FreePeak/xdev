...
| M1 | Provider layer: 2 adapters (Anthropic + OpenAI Responses), SSE, watchdogs, unified events, error-classifier taxonomy, failed-request dumps, partial-tool-call stubs never executed | streamed chat in `print` mode | Not started | [#2](https://github.com/FreePeak/xdev/issues/2) |
...
| M3 | Agent loop + 4 tools + OutputSink + env hardening + approvals + todo tool (omp 9-op port) + arg-repair pipeline + file-freshness guard + backgrounded bash | real coding tasks end-to-end | Not started | [#4](https://github.com/FreePeak/xdev/issues/4) |
...
*Last updated: 2026-09-09 (second-wave cross-harness research integrated: 7 primary-source docs under docs/research/ — omp/pi/Claude Code/opencode/hermes internals + omp todos + omp context-resilience; goal-4 pi-tool-count correction, parity-matrix additions, scope bullets, milestone deltas, key decisions, links)*
