...
| M5 | Compaction (threshold + overflow + promotion + method ladder), retry/failover + partial-stream turn recovery | 200k-token sessions survive | Not started | [#6](https://github.com/FreePeak/xdev/issues/6) |
...
*Last updated: 2026-09-09 (M4 TUI shipped: `xdev tui` Grok-styled interactive mode end-to-end — theme engine (slot vocabulary, quantization, GrokNight/GrokDay), streaming blocks, editor, status line, Esc/Ctrl+C semantics, OSC 12 cursor, tmux-verified live incl. tools + cancel + persistence; binary installed at ~/.local/bin/xdev (11.6 MB RSS boot); M3 status: print mode complete; M4 row updated with deferrals to #5)*
