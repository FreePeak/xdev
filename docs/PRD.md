...
*Last updated: 2026-09-11 (M10 resume/fork/dump slice 6a7d8e2: `-resume <prefix>` + breadcrumb-first `-continue` + TUI `/fork` `/dump` `/resume` with `parentSession` header; welcome screen v2, #19: ANSI-shadow block logo respelled "XDev" — lowercase e/v hand-cut from the caps-only font's E/V glyphs at a raised x-height baseline; one-logo-at-every-size contract replaces the two-tier logoArt (identical art at any fitting size, hidden when it can't fit); digital-rain backdrop replaced by Conway's Game of Life — random soup on a toroidal grid, B3/S23 at ~8fps while the welcome screen is visible, birth-mutations keep still lifes from freezing; CRT phosphor greens and binary 'xdev' tagline unchanged. Prior rain-era chrome: 0e32757 + f87f10e + df01150, dot restored in c993db3.)*
