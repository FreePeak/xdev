...
*Reference: `leankg/AGENTS.md`, `leankg/docs/prd.md` v4.12.2 (#414) · Milestone: M12 (memory) follow-on · Date: 2026-09-15 · Status: **proposed (plan; no code written)***
...
| **X2** | **Fail-fast on an unresolvable scope** | Companion to X1: with LeanKG's scope matrix, an unset bank silently retargets `leankg-shared`. Refuse to start the backend with a scope that cannot name a bank, or derive it and log it once at startup (`print.go:488-492` already has a warn sink for exactly this class of message). | `cmd/xdev/print.go:1263+`, `buildHindsightMemory` |
...
| **X4** | **Project selector** | LeanKG routes multi-project servers by `?project=` (`cmd/leankg/multiproject.go:41-53`); xdev never sends it, so every write lands on the server's default project. Send the memory root / project path and honor the unknown-project rejection instead of swallowing it. | `internal/memory/hindsight.go:1156-1160` (`bankPath`), `do` |
