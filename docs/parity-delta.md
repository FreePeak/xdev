...
| `/goal <verb>` dropped its arguments — `/goal create …` looked dead | user-reported | GoalOps wired to the live GoalState; unknown verbs report the grammar |
| `/goal` required a verb; `/goal <what I want>` was a usage error | user-reported | the argument IS the objective — `/goal ship the exporter` sets it and starts the first turn. A bare `/goal` shows it; `complete`/`drop` close it; the older `create <objective>` spelling still works |
| `/goal` answered `error: goal not wired` in every TUI session | user-reported | the goal tool was registered only in `runPrint` (the TUI builds its own agent), so `agent.GoalStateOf` found nothing. The TUI now registers it next to its store, binds it by construction, and `/goal <objective>` names the objective (no auto-created placeholder in the TUI: an active goal blocks /vibe) |
...
(no in-process JS), browser is attach-first CDP (a running Chrome, else one xdev
launches on its own profile and stops again after 5 idle minutes; the user's
browser is never closed), TTS/computer/DAP are shell-out backends.
