...
func (t *TodoTool) Description() string {
	return "Phased task list. Ops: init (list:[{phase,items}] or flat items) replaces the list; start(task); " +
		"done|drop(task|phase, or neither = the whole list); block(task|phase, reason?); unblock(task|phase); " +
		"append(phase, items); view; rm(task|phase, or neither = clear). After every change the earliest " +
		"pending task auto-promotes to in_progress; blocked tasks never auto-promote. Reference tasks by " +
		"their exact content text, never invented IDs. Batch todo calls with real work."
}
...
func (t *TodoTool) Parameters() json.RawMessage {
  ...
  "properties": {
    ...
    "list": {
      "type": "array",
      "description": "init: phased payload; replaces the existing list",
      "items": {
        "type": "object",
        "required": ["phase", "items"],
        "properties": {
          "phase": {"type": "string", "description": "Short noun-phrase phase name"},
          "items": {"type": "array", "minItems": 1, "items": {"type": "string"}, "description": "Task contents"}
        }
      }
    },
    "task": {"type": "string", "description": "Exact task content (never an ID)"},
    "phase": {"type": "string", "description": "Exact phase name"},
    "items": {"type": "array", "items": {"type": "string"}, "description": "Task contents (flat init / append)"},
