...
func (t *EditTool) Parameters() json.RawMessage {
  ...
  "properties": {
    "input": {
      "type": "string",
      "description": "One patch for one file. Send the patch as input. Every body row under a PUT begins with exactly one \"+\" (that prefix is transport: \"++x\" writes \"+x\", a lone \"+\" is a blank line); a leading \"-\" is plain content, never a deletion. Line 1 is a header naming the file — the path INSIDE the brackets, \"[src/a.go#1a2b]\" or \"[src/a.go]\" (a #tag is the one a read or a previous edit of this file printed; never invent it, and without one the lines must still match what you read). Each op line names 1-based lines of the file as the previous op left it:\n  PUT 3.=5:   replace lines 3-5 with the rows that follow\n  PUT 3:      replace one line (no rows deletes it)\n  PUT <3:     insert the rows before line 3, keeping line 3\n  PUT >3:     insert the rows after line 3, keeping line 3\n  CUT 3       delete one line\n  REM 3.=5:   delete a range\n  MV new.go   rename the file\nRows may be indented; the whitespace after the \"+\" is kept verbatim. This is not a unified diff — never send \"@@\" hunks or \"-\" rows.\nExample:\n[src/a.go#1a2b]\nPUT 3.=4:\n+func main() {\n+\txdev.Run()\nREM 9\nMV cmd/main.go"
    }
  }
}`)
}
...
func (t *EditTool) Execute(ctx context.Context, args json.RawMessage) (Result, error) {
	...
	if fail != "" {
		// decodeEditArgs and parseHashline already lead their errors with
		// "edit: " (the shape a live session sees most often); without this
		// the model reads "edit: edit: line 2: ...".
		fail = strings.TrimPrefix(fail, "edit: ")
