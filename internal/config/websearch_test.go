...
`)
	...
	// while keys and the timeout it never mentions must survive. The block
	// carries API keys, so the layer is a file the user chose, not a clone's
	// (#114: webSearch is profile-owned).
	duck := writeFile(t, filepath.Join(t.TempDir(), "duck.yml"), `
webSearch:
  providers: [duckduckgo]
`)
	s, err = LoadSettings(cwd, []string{duck})
	if err != nil {
		t.Fatal(err)
	}
	if len(s.WebSearch.Providers) != 1 || s.WebSearch.Providers[0] != "duckduckgo" {
		t.Fatalf("providers after the overlay = %v", s.WebSearch.Providers)
	}
	if s.WebSearch.Timeout != "5s" || s.WebSearch.APIKeys["tavily"] != "tvly-one" {
		t.Fatalf("overlay lost untouched webSearch keys: %+v", s.WebSearch)
	}
