...
// RecallTool is the model-facing polyphonic recall.
type RecallTool struct{ Mem *Mnemopi }

// RetainTool records one memory explicitly.
type RetainTool struct{ Mem *Mnemopi }

// ReflectTool runs the synthesis pass over the recent memories.
type ReflectTool struct{ Mem *Mnemopi }
...
func (t *ReflectTool) mem() *Mnemopi {
	if t == nil || t.Mem.Off() {
		return nil
	}
	return t.Mem
...
func (t *RecallTool) Execute(ctx context.Context, args json.RawMessage) (tool.Result, error) {
	m := t.mem()
	if m == nil {
		return offErr(RecallToolName)
	}
	var a struct {
...
// retainItem is one memory in the retain batch.
type retainItem struct {
	Text   string
	Source string
}

func (t *RetainTool) Execute(ctx context.Context, args json.RawMessage) (tool.Result, error) {
	m := t.mem()
	if m == nil {
		return offErr(RetainToolName)
	}
	var a struct {
		Items []struct {
	...
	items := make([]retainItem, 0, len(a.Items)+1)
	for _, it := range a.Items {
		items = append(items, retainItem{Text: it.Content, Source: it.Context})
	}
	if strings.TrimSpace(a.Text) != "" {
		items = append(items, retainItem{Text: a.Text, Source: a.Context})
	}
	if len(items) == 0 {
...
func (t *ReflectTool) Execute(ctx context.Context, args json.RawMessage) (tool.Result, error) {
	m := t.mem()
	if m == nil {
		return offErr(ReflectToolName)
	}
	var a struct {
