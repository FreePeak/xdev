...
func pressRosterKey(app *App, key tcell.Key, r rune) bool {
...
func TestHubRosterOverlayKeys(t *testing.T) {
	...
	// starts at 0).
	if !pressRosterKey(app, tcell.KeyEnter, 0) {
	...
	if pressRosterKey(app, tcell.KeyDown, 0) || pressRosterKey(app, tcell.KeyRune, 'k') {
...
func TestHubRosterKillParkReviveKeys(t *testing.T) {
	...
	if pressRosterKey(app, tcell.KeyRune, 'x') {
...
func TestHubRosterOverlayRenders(t *testing.T) {
	...
	pressRosterKey(app, tcell.KeyEnter, 0) // transcript of hub-1
	app.mu.Lock()
	app.drawHubRoster(20)
	app.mu.Unlock()
	scr.Show()
